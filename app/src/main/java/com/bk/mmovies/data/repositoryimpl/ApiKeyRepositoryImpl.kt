...
/*
  override suspend fun getTokenValidationResult(): TokenValidationResult {
        val result: ApiCallResult<TokenValidityDto> =
        ...
        return when (result) {
            is ApiCallResult.Failure -> {
                when (result.error) {
                    is NetworkError.ExceptionError, NetworkError.EmptyBody -> {
                        TokenValidationResult.Failure("Couldn't verify API key, please check your network")
                    }
                    ...
                    is NetworkError.HttpError -> {
                        ...
                        val apiStatusCode = errorDto?.statusCode ?: 0
                        if (httpCode == 401 && (apiStatusCode == 7 || apiStatusCode == 10)) {
                            TokenValidationResult.Failure(errorDto?.message ?: "Invalid API key")
                        } else {
                            TokenValidationResult.Failure("Couldn't verify API key")
                        }
                    }
                }
            }
            ...
            is ApiCallResult.Success<TokenValidityDto> -> {
                ...
                if (isTokenValid != null) {
                    ...
                    } else TokenValidationResult.Failure(
                        "Invalid API key"
                    )
                } else {
                    TokenValidationResult.Failure("Couldn't verify API key")
